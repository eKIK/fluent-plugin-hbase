...
[row_key_name (optional)] The key where each event's HBase row key is fetched from.

The key should most probably not be included in the 'fields_to_columns_mapping',
unless you want to have your row key stored as a cell value as well.

The default is to use a random UUID as row key.
